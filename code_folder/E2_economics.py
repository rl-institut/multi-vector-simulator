...
class economics:
    def get_costs(dict_asset, economic_data):
        if isinstance(dict_asset, dict) \
            ...
            if helpers.all_list_in_dict(dict_asset, ['lifetime_capex_var', 'capex_fix', 'optimizedAddCap']) == True \
                    and dict_asset['optimizedAddCap']['value'] > 0:
                # total investments including fix prices
                costs_investment = dict_asset['optimizedAddCap']['value'] * dict_asset['lifetime_capex_var']['value'] + dict_asset['capex_fix']['value']
                costs_total = helpers.add_costs_and_total(dict_asset, 'costs_investment', costs_investment, costs_total)

            if helpers.all_list_in_dict(dict_asset, ['capex_var', 'capex_fix', 'optimizedAddCap']) == True \
                    and dict_asset['optimizedAddCap']['value'] > 0:
                # investments including fix prices, only upfront costs at t=0
                costs_upfront = dict_asset['optimizedAddCap']['value'] + dict_asset['capex_var']['value'] + dict_asset['capex_fix']['value']
            ...
            if helpers.all_list_in_dict(dict_asset, ['annual_total_flow', 'lifetime_opex_var']) == True:
                costs_opex_var = dict_asset['lifetime_opex_var']['value'] * dict_asset['annual_total_flow']['value']
            ...
            if helpers.all_list_in_dict(dict_asset, ['annual_total_flow', 'lifetime_opex_var', 'installedCap', 'optimizedAddCap']) == True:
