...
class data_input:
    def all(user_input):
        ...
        project_data, simulation_settings, economic_data = get_values.project_data(
            user_input
        )
        ...
        for asset_group in included_assets.keys():
            if asset_group == "sectors":
                ...
                for asset_group_item in included_assets[asset_group]:
                    dict_asset_group_item = get_values.sectors(
                        user_input, asset_group_item
                    )
                ...
                for asset_group_item in included_assets[asset_group]:
                    ...
                    dict_asset_group_item = get_values.components(
                        user_input, asset_group_item
                    )
                    dict_values.update(dict_asset_group_item)
                    pass
...
class get_values:
        ...
        simulation_settings.update(
            {
                "index": pd.date_range(
                    ...
                    + pd.DateOffset(
                        days=simulation_settings["evaluated_period"], hours=-1
                    ),
    ...
    def sectors(user_input, asset_group_item):
        ...
        if asset_group_item == "Electricity":
            ...
            dict_sector = {}
            logging.error(
                'Input data extraction of sector "%s" not defined!', asset_group_item
            )
            pass
    ...
    def components(user_input, asset_group_item):
        ...
        if asset_group_item == "PV plant":
            ...
            logging.error(
                'Input data extraction of asset "%s" not defined!', asset_group_item
            )
            dict_value_specific = assets.wind(user_input, asset_group_item)
        elif asset_group_item == "transformer_station":
            dict_value_specific = assets.transformer_station(
                user_input, asset_group_item
            )
            ...
            dict_value_specific = assets.electricity_storage(
                user_input, asset_group_item
            )
        else:
            logging.critical(
                "Component %s can not be simulated, as it is not defined!",
                asset_group_item,
            )
    ...
    def demand(user_input, dict_excel_data):
        ...
        all_titles = {
            "Demand side management (DSM)***": "dsm",
            "File (csv)": "file_name",
        }
                ...
                if item in data[demand_number].keys():
                    if item == "Demand side management (DSM)***":
                        if data[demand_number][item] == "Yes":
                            ...
                            logging.warning(
                                "Input error: %s\n"
                                "Demand side management of an demand series can only be (Yes/No).",
                                dict_excel_data["tab_name"],
                            )

                    else:
                        dict_demands[demand_name].update(
                            {
                                all_titles[item]: data[demand_number][item],
                                "parent": "demand",
                                "label": demand_name,
                                "type": "sink",
                            }
...
class assets:
    def electricity_sector(user_input):
        ...
        dict_sector = {
            "electricity_grid": electricity_sector,
            "electricity_demand": demand_profiles,
        }

        # Retrieving cost info
        dict_costs = helpers.cost_info(
            user_input, tab_name, ["electricity_grid", "transformer_station"]
        )
    ...
    def transformer_station(user_input, tab_name):  #
        ...
        dict_asset = helpers.parameters(
            user_input, dict_excel_data["transformer_station"]
        )
        ...
        transformer_station.update(
            {
                "electricity_cost_fix_annual": data["Fix annual cost"][
                    "Electricity supplier"
                ]
                + data["Fix annual cost"]["DSO"]
                + 12
                * (
                    data["Fix monthly cost"]["Electricity supplier"]
                    + data["Fix monthly cost"]["DSO"]
                ),
                "electricity_price_var_kWh": data["Variable cost per kWh supplied"][
                    "Electricity supplier"
                ]
                + data["Variable cost per kWh supplied"]["DSO"],
                "electricity_price_var_kW_monthly": data[
                    "Variable cost per kW peak demand/month "
                ]["Electricity supplier"]
    ...
    def pv(user_input, asset_group_item):
        ...
        ## Retrieving all data concerning the electricity sector
        dict_asset = {"pv_installation": {}, "solar_inverter": {}}
        # Parameters solar panels
        dict_asset_parameters = helpers.parameters(
            user_input, dict_excel_data["pv_installation"]
        )
        ...
        dict_asset["pv_installation"].update(
            {
                "file_name": data["File"]["Historical electricity generation"],
                "type": "source",
            }
        )
        # Parameters solar inverter
        dict_asset_parameters = helpers.parameters(
            user_input, dict_excel_data["solar_inverter"]
        )
        dict_asset["solar_inverter"].update(dict_asset_parameters)
        dict_asset["solar_inverter"].update({"type": "transformer"})
        # Retrieving cost info
        dict_costs = helpers.cost_info(
            user_input, tab_name, ["pv_installation", "solar_inverter"]
        )
        for key in dict_costs.keys():
            dict_asset[key].update(dict_costs[key])
            dict_asset[key].update({"label": key, "parent": "pv_plant"})
    ...
    def electricity_storage(user_input, asset_group_item):
        ...
        dict_excel_data = {
            ...
            "technical_data_charge_controller": {
                "tab_name": tab_name,
                "first_row": 37,
                "number_of_rows": 3,
                "column_string": "B:C",
        ...
        data = read_template.read_excel_tab(
            user_input, dict_excel_data["economic_data"]
        )
        for sub_asset in name_dict_sub_assets.keys():
            for item in all_titles.keys():
                dict_asset[sub_asset].update(
                    {all_titles[item]: data[item][name_dict_sub_assets[sub_asset]]}
                )
        # Technical data
        data = read_template.read_excel_dict(
            user_input, dict_excel_data["technical_data_storage_unit"]
        )
        data = data["Value"]
        dict_asset["charging_power"].update(
            {
                "crate": data["Inflow C-rate"],
        ...
        dict_asset["capacity"].update(
            {
                "soc_min": data["Min. charge"] / 100,
                "soc_max": data["Max. charge"] / 100,
                "self_discharge": data["Self-discharge / Charge loss per timestep"]
                / 100,
                "type": "storage",
            }
        )
        ...
        dict_asset["discharging_power"].update(
            {
                "efficiency": data["Outflow efficiency"] / 100,
        ...
        dict_parameters_charge_controller = helpers.parameters(
            user_input, dict_excel_data["technical_data_charge_controller"]
        )
        dict_asset["charge_controller"].update(dict_parameters_charge_controller)

        # Retrieving cost info
        dict_costs = helpers.cost_info(
            user_input, tab_name, list(name_dict_sub_assets.keys())
        )
        for key in dict_costs.keys():
            dict_asset[key].update(dict_costs[key])
            dict_asset[key].update({"label": key, "parent": "electricity_storage"})

        dict_asset = {"electricity_storage": dict_asset}
        dict_asset["electricity_storage"].update(
            {"label": "electricity_storage", "type": "storage"}
        )
        return dict_asset
