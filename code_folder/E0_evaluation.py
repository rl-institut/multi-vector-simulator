...
class evaluation:
    def evaluate_dict(dict_values, results_main, results_meta):
        dict_values.update({'kpi': {'cost_matrix': pd.DataFrame(columns=['label',
                                                                         'costs_total',
                                                                         'costs_om',
                                                                         'costs_investment',
                                                                         'costs_upfront',
                                                                         ...
                                                                         'annuity_total',
                                                                         'annuity_om']),
                                    'scalar_matrix': pd.DataFrame(columns=['label', 'optimizedAddCap', 'total_flow', 'annual_total_flow', 'peak_flow', 'average_flow']),
                                    'scalars': {}}})
        bus_data={}
        ...
        for sector in dict_values['energyStorage']:
            for storage in dict_values['energyStorage'][sector]:
                ...
                helpers.store_result_matrix(dict_values['kpi'], dict_values['energyStorage'][sector][storage])
        ...
        for asset in dict_values['energyConversion']:
            ...
            helpers.store_result_matrix(dict_values['kpi'], dict_values['energyConversion'][asset])
        ...
        for group in ['energyProduction', 'energyConsumption']:
            for sector in dict_values[group]:
                for asset in dict_values[group][sector]:
                    ...
                    helpers.store_result_matrix(dict_values['kpi'], dict_values[group][sector][asset])
            ...
            for key in dict_kpi[kpi_storage].columns.values:
                ...
                if key in dict_asset:
                    if isinstance(dict_asset[key], str):
                        ...
                        asset_result_dict.update({key: round(dict_asset[key]['value'], round_to_comma)})

            asset_result_df = pd.DataFrame([asset_result_dict])

            dict_kpi.update({kpi_storage: dict_kpi[kpi_storage].append(asset_result_df, sort=False)})
        return