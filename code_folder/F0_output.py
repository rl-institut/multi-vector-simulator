...
class output_processing():
    def evaluate_dict(dict_values):
        ...
        for sector in dict_values['project_data']['sectors']:
            ...
            total_demand = pd.Series([0 for i in dict_values['simulation_settings']['time_index']],
                                                   index = dict_values['simulation_settings']['time_index'])

            for asset in dict_values['energyConsumption'][sector_name]:
                total_demand = total_demand + dict_values['energyConsumption'][sector_name][asset]['flow']

            #todo this should actually link to C0: helpers.bus_suffix
            dict_values['optimizedFlows'][sector_name+' bus']['Total demand ' + sector_name] = total_demand

            plots.flows(dict_values['simulation_settings'], dict_values['project_data'],
                        dict_values['optimizedFlows'][sector_name+' bus'], sector, 14)
            plots.flows(dict_values['simulation_settings'], dict_values['project_data'],
