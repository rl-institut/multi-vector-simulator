...
class output_processing():
    def evaluate_dict(dict_values):
        ...
        for sector in ['electricity', 'heat']:
            results_timeseries = {} #pd.DataFrame(index=dict_values['settings']['index'])
            results_scalars_assets = {} #pd.DataFrame(columns=['asset', 'name', 'value'])
            results_scalars_other = {}

            for item in dict_values.keys():
                helpers.write_results(dict_values[item], results_scalars_assets, results_scalars_other, results_timeseries)
                for subitem in dict_values[item].keys():
                    helpers.write_results(dict_values[item][subitem], results_scalars_assets, results_scalars_other, results_timeseries)
                    if isinstance(dict_values[item][subitem], dict):
                        for subsubitem in dict_values[item][subitem].keys():
                            helpers.write_results(dict_values[item][subitem][subsubitem], results_scalars_assets, results_scalars_other, results_timeseries)
...
class helpers:
    def write_results(dict_asset, results_scalars_assets, results_scalars_other, results_timeseries):
        if isinstance(dict_asset, dict):
            ...
            if ('input_bus_name' in dict_asset) or ('output_bus_name' in dict_asset):
                ...
                helpers.write_scalars_assets(dict_asset, results_scalars_assets, results_timeseries)
    ...
    def write_scalars_assets(dict_asset, results_scalars_assets, results_timeseries):
        keyword_list = ['type',
                        'timeseries_soc',
