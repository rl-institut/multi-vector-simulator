...
class read_template:
    def overview_energy_system(user_input):
        ...
        dict_of_overview_info = {'sectors': {'title': 'Energy sectors/carriers to be considered',
                                             'tab_name': tab_name,
                                             'first_row': 14,
                                             'number_of_rows': 6,
                                             'column_string': 'B:C',
        ...
        included_assets = {} #todo - if energy provider includes electricity etc, it actually adds anther asset: transformer station, ie.
        for asset_group_name in dict_of_overview_info.keys():
            # Reads and checks info on all asset groups. Returns list of strings.
            included = read_template.get_included_assets(user_input, tab_name, asset_group_name, dict_of_overview_info[asset_group_name])
