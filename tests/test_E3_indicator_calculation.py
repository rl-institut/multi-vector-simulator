...
class TestGeneralEvaluation:
    ...
    def test_total_renewable_and_non_renewable_origin_of_each_sector(self):
        ...
        assert "Total internal renewable generation" in dict_renewable_energy_use[KPI_DICT][KPI_SCALARS_DICT]
        assert "Total renewable energy use" in dict_renewable_energy_use[KPI_DICT][KPI_SCALARS_DICT]
        assert "Total internal non-renewable generation" in dict_renewable_energy_use[KPI_DICT][KPI_SCALARS_DICT]
        assert "Total non-renewable energy use" in dict_renewable_energy_use[KPI_DICT][KPI_SCALARS_DICT]
...
class TestTechnicalParameters:
    def test_weighting_for_sector_coupled_kpi_unknown_sector(self):
        with pytest.raises(ValueError):
            E3.weighting_for_sector_coupled_kpi(dict_weighting_unknown_sector, a_kpi_name)
    ...
    def test_weighting_for_sector_coupled_kpi_one_sector(self):
        ...
        assert dict_weighting_one_sector[KPI_DICT][KPI_SCALARS_DICT][a_kpi_name] == numbers[1]
    ...
    def test_renewable_share_one_sector(self):
        ...
        assert "Renewable share" in dict_renewable_energy_use[KPI_DICT][KPI_UNCOUPLED_DICT]
        assert "Electricity" in dict_renewable_energy_use[KPI_DICT][KPI_UNCOUPLED_DICT]["Renewable share"]
        assert dict_renewable_energy_use[KPI_DICT][KPI_UNCOUPLED_DICT]["Renewable share"]["Electricity"] == exp
        assert dict_renewable_energy_use[KPI_DICT][KPI_SCALARS_DICT]["Renewable share"] == exp
