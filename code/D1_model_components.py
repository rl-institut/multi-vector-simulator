...
class call_component:
    ...
    def transformer(model, dict_asset, **kwargs):
        helpers.check_optimize_cap(model, dict_asset,
                                   ...
                                   func_optimize=define_oemof_component.transformer_constant_efficiency_optimize, **kwargs)
        return

    def storage(model, dict_asset, **kwargs):
        helpers.check_optimize_cap(model, dict_asset,
                                   func_constant=define_oemof_component.storage_fix,
                                   func_optimize=define_oemof_component.storage_optimize, **kwargs)
        return
...
class helpers():
    def check_optimize_cap(model, dict_asset, func_constant, func_optimize, **kwargs):
        ...
        if dict_asset['optimizeCap'] == False:
            ...
            logging.debug('Defined asset %s as %s (fix capacity)', dict_asset['label'], dict_asset['type_oemof'])

        elif dict_asset['optimizeCap'] == True:
            func_optimize(model, dict_asset, **kwargs)
            logging.debug('Defined asset %s as %s (to be optimized)', dict_asset['label'], dict_asset['type_oemof'])
        else:
...
class define_oemof_component():
    ...
    def storage_fix(model, dict_asset, **kwargs):
        storage = solph.components.GenericStorage(
            ...
            inputs={kwargs['busses'][dict_asset['input_bus_name']]: solph.Flow(
                nominal_value= dict_asset['discharging_power']['installedCap']['value'], #limited through installed capacity, NOT c-rate
    ...
    def storage_optimize(model, dict_asset, **kwargs):
        storage = solph.components.GenericStorage(
            ...
            investment=solph.Investment(ep_costs=dict_asset['capacity']['simulation_annuity']['value']),
            inputs={kwargs['busses'][dict_asset['input_bus_name']]: solph.Flow(
            ...
            invest_relation_output_capacity=dict_asset['discharging_power']['crate']['value']
        ...
        source_non_dispatchable = solph.Source(label=dict_asset['label'],
                                 outputs={kwargs['busses'][dict_asset['output_bus_name']]:
                                              solph.Flow(label=dict_asset['label'],
                                                         ...
                                                         existing = dict_asset['installedCap']['value'],
                                                         investment=solph.Investment(
                                                             ep_costs=dict_asset['simulation_annuity']['value'] / dict_asset['timeseries_peak']['value']),
                                                         variable_costs=dict_asset['opex_var']['value'] / dict_asset['timeseries_peak']['value']
                                                         )})
