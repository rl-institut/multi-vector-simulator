    ...
    def get_costs(dict_asset, economic_data):
        if isinstance(dict_asset, dict) and not (
            ...
            if (
                ...
                costs_total = helpers.add_costs_and_total(
                    dict_asset, "costs_investment", costs_investment, costs_total
                )

            if (
                helpers.all_list_in_dict(
                    dict_asset, ["capex_var", "capex_fix", "optimizedAddCap"]
                )
                == True
                and dict_asset["optimizedAddCap"]["value"] > 0
            ):
            ...
            if (
                helpers.all_list_in_dict(
                    dict_asset, ["annual_total_flow", "lifetime_opex_var"]
                )
                == True
            ):
                costs_opex_var = (
                ...
                cost_om = helpers.add_costs_and_total(
                    dict_asset, "costs_opex_var", costs_opex_var, cost_om
                )

            # todo actually, price is probably not the label, but opex_var
            if (
                helpers.all_list_in_dict(dict_asset, ["price", "annual_total_flow"])
                == True
            ):
                costs_energy = (
                    dict_asset["price"]["value"]
                    * dict_asset["annual_total_flow"]["value"]
                )
                cost_om = helpers.add_costs_and_total(
