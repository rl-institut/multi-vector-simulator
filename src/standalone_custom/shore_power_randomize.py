...
def randomize_shore_power(
    times, shore_power, filename="randomized_shore_power_demand.csv", docks=1
):
    r"""
    ...
    * Write more tests - for convenient testing this function might have to
    ...
    """
    total_shore_power = pd.DataFrame(
        [0 for i in range(0, len(times))], index=times, columns=["count"]
    )
    ...
    for column in shore_power.columns:
        ...
        while correct_combination != True:
            ...
            time_of_events = (
                pd.Series(
                    [shore_power[column]["Power"] for i in range(0, len(times))],
                    index=times,
                )
                .reset_index()["index"]
                .sample(
            ...
            time_periods = time_of_events.apply(
                lambda x: pd.date_range(
                    ...
                    + pd.DateOffset(hours=int(shore_power[column]["Duration"] - 1)),
                    freq="h",
                )
            ).values
            joined_periods = time_periods[0]
            for i in range(1, len(time_periods)):
                joined_periods = joined_periods.join(time_periods[i], how="outer")

            expected_length = (shore_power[column]["Duration"]) * len(time_of_events)
            if len(joined_periods) != expected_length or not times.contains(
                joined_periods[-1]
            ):
            ...
            availability = (
                total_shore_power["count"]
                .loc[joined_periods]
                .apply(lambda x: True if x < docks else False)
            )
            ...
            total_shore_power.loc[joined_periods, f"temp_{column}"] = shore_power[
                column
            ]["Power"]
    ...
    hours_without_docking = total_shore_power.loc[
        total_shore_power["count"] == 0.0
    ].index
