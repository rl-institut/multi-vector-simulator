...
def source_non_dispatchable_fix(model, dict_asset, **kwargs):
    ...
    source_non_dispatchable = solph.components.Source(label=dict_asset[LABEL], outputs=outputs)
...
def source_dispatchable_optimize(model, dict_asset, **kwargs):
    ...
    if TIMESERIES_NORMALIZED in dict_asset:
        ...
        source_dispatchable = solph.components.Source(label=dict_asset[LABEL], outputs=outputs,)
    else:
